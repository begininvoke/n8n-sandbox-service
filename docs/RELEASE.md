...
The sandbox image is published before the golden-build tarball is packaged, because
packaging verifies that the pinned `sandbox_image.ref` resolves.
...
Schema v3 changed two things for consumers. The guest userspace now comes from the
sandbox image instead of the Firecracker CI Ubuntu squashfs, so
`build-rootfs-template.sh` requires `SANDBOX_IMAGE` or `SANDBOX_ROOTFS_TAR` and
`firecracker-ci-assets.sh` no longer downloads or exports a squashfs. And the
`service_version` and `sandbox_version` fields are replaced by a single `version`,
matching the one version all images now share.
...
Packaging resolves the ref against the registry (crane or
`docker buildx imagetools`) and fails if it does not exist. Set
`SANDBOX_IMAGE_VERIFY=0` to skip that lookup when packaging from a tree whose
version has not been published yet, as the bundle self-test does.
...
3. Packages the golden-build tarball pinning the ACR sandbox candidate, then
   creates a GitHub prerelease (`service/v{version}`) with the tarball attached
