...
1. The runner is waiting on the VMM process — jailer execs Firecracker in place, so that process lives exactly as long as the guest — and reads the exit as a crash unless it asked for it, which a generation counter bumped before every deliberate kill decides
2. The dead sandbox is torn down and **hands its slot back** immediately, leaving it in the state an idle stop leaves behind minus a usable snapshot: stopped, no slot, files intact. Nothing is recovered until a request arrives, so a crashed sandbox nobody touches again costs only disk
3. The next request finds the sandbox not running and drives the ordinary wake path, which **cold boots the sandbox's own rootfs** instead of restoring its snapshot, replaying the boot parameters recorded when it was created
4. That request is then failed with `409 sandbox_restarted` rather than proxied, because the recovery cannot restore what was in memory: processes an earlier execution started, and the daemon's in-memory execution history. The retry succeeds. See [API.md](API.md#http-409-sandbox_restarted--the-sandbox-came-back-without-its-memory)

**Never restore a stale snapshot.** Restoring a memory image onto a rootfs the guest has written to since that snapshot was taken corrupts the filesystem silently: the image carries the guest's cached filesystem metadata, the checksum covers only the state file, and nothing detects the mismatch. So a sandbox is pinned to cold boot from the moment its snapshot is restored, and only a clean stop — which pauses the guest before snapshotting it — pairs a fresh snapshot with the disk again and lifts the pin.

Cold boot was chosen over recreating the sandbox from the golden snapshot because it keeps the files and is faster: it skips the rootfs and snapshot clones that are ~72% of a create, and unlike a snapshot restore it does not scale with the flavor's memory size. Recreating from the golden snapshot remains the fallback if cold boot ever measures worse.
...
2. The runner learns of the death from a `docker events` stream filtered to `die` on its own containers. A death it asked for is not a crash, so every deliberate stop and remove is recorded before the call that causes it and matched against the event; exit codes are never consulted, because a guest that exits `0` on its own has still lost everything it was running
3. A sandbox marked that way is reported as not running until the runner has re-admitted it, which is what drives the wake path over a container that already looks healthy. That is not a formality: the restarted container may hold a **new IP**, and its network policy still points at the old one
...
Losing the event stream is the one silent failure here, since containers keep working while crashes stop being reported, so the watcher reconnects for the life of the runner. A death missed during a reconnect goes unreported: the container usually comes back on the address it had, so the sandbox keeps working and nothing tells the client its in-memory state is gone. That gap is why the watcher's reconnects matter more than they look.
