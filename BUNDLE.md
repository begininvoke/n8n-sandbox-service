...
(`sandbox_image.ref`, from `VERSION`). Snapshot create still injects the bundle's
`bin/sandbox-daemon` as `/sandbox-daemon` (PID 1). The tree is extracted as root
with `--numeric-owner`, so image ownership and setuid bits survive; `/home/user`
stays `1000:1000` so the daemon (which drops privileges) can write the workspace.
